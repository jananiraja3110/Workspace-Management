...
// ── StatusSelect / PrioritySelect — native selects, no clipping issues ────────
...
function PrioritySelect({ value, onChange, disabled }) {
  ...
  return (
    <div className="relative inline-flex items-center">
      <Flag className={`absolute left-2 w-3 h-3 pointer-events-none ${pc.text}`} style={{ fill: pc.hex, color: pc.hex }}/>
      <select value={value} onChange={e => onChange(e.target.value)} disabled={disabled}
        onClick={e => e.stopPropagation()}
...
function BoardDnd({ statuses, tasks, users, isAdminOrHR, spaceId, onUpdate, onDelete, onOpen, setTasks }) {
  ...
  async function handleDragEnd({ active, over }) {
    ...
    if (!targetCol) return;
    if (targetCol.id === moved.status && String(over.id) === taskId) return;
